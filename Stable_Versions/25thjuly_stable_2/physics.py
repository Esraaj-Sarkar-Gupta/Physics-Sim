...
def collide(p1 , p2):
    ...
    end_time = time.time()
    log(f"[MATHS]: Collision >> {solution} in time {end_time - start_time:.4f}")
    ...
    momentum_i = c
    ke_i = d
    momentum_f = (p1.mass * mag(p1.v)) + (p2.mass * mag(p2.v))
    ke_f = (p1.mass * mag(p1.v)**2) + (p2.mass * mag(p2.v)**2)
    
    error_collision = False
    
    if momentum_i != momentum_f:
        log(f"[MATH]: Collision >> [ERROR]: Momentum is not conserved! Difference = {momentum_f - momentum_i}")
        error_collision = True
    
    if ke_i != ke_f:
        log(f"[MATH]: Collision >> [ERROR]: Kinetic energy is not conserved! Difference = {ke_f - ke_i}")
        error_collision = True
...
class body:
    ...
    def update(self, a, del_t):
        ...
        try:
            ...
            log(f"[ERROR]: Update fails to run due to collision. a = {a}")
